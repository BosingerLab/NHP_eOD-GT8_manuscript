...
while($ln = <IN>)
{
	...
	my @arr = split("\t",$ln);	
	$db{$arr[0]} = $ln;
	my $cdr3_len = length($arr[50]);

	$clonotype{$cdr3_len}{$arr[0]}{'V'} = $arr[7];
	$clonotype{$cdr3_len}{$arr[0]}{'J'} = $arr[9];
	#$clonotype{$arr[0]}{'junc'} = $arr[28];
	#$clonotype{$arr[0]}{'junc_len'} = $arr[29];
	$clonotype{$cdr3_len}{$arr[0]}{'cdr3'} = $arr[50];
...
foreach my $len(sort keys %clonotype)
{
	...
	foreach my $s1 (sort keys %{$clonotype{$len}})
	{
		...
		foreach my $s2 (sort keys %{$clonotype{$len}})
		{
        	#if($s1 eq $s2) {next;}
			# print "$s1\t$s2\n";
			my $hv_check = compare_arr($clonotype{$len}{$s1}{'V'},$clonotype{$len}{$s2}{'V'});
			if($hv_check)
			{
				my $hj_check = compare_arr($clonotype{$len}{$s1}{'J'},$clonotype{$len}{$s2}{'J'});
				if($hj_check)
				{
					#print "$s1 $clonotype{$s1}{'cdr3'} $s2 $clonotype{$s2}{'cdr3'}\n";  
					#my $hcdr3_check = compare_cdr3($clonotype{$s1}{'junc'},$clonotype{$s2}{'junc'});
					my $hcdr3_check = compare_cdr3($clonotype{$len}{$s1}{'cdr3'},$clonotype{$len}{$s2}{'cdr3'});
					if($hcdr3_check)
					{		
						++$v100{$_} for $s1,$s2;
			    		$uf100->union($s1,$s2);
...
open OUT,">$ARGV[0].clones_v2.tab" or die $!;
